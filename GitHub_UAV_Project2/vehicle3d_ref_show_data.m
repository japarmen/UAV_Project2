function vehicle3d_ref_show_data(t,x,u,x_ref,printFigs,filename,ifig)
    ...
    figure(11);
    plot(t{1},dt,t{1},Ts*ones(size(t{1})));
    xlabel('$$t$$ [s]');
    grid on;
    legend('$$\Delta t$$','$$T_{s, avg}$$');
    title(['stats on $$\Delta t$$: avg = ' num2str(mean(dt*1e3)) ' ms; min = ' num2str(min(dt*1e3)) ' ms; max = ' num2str(max(dt*1e3)) ' ms; ']);
